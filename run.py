...
def resolve_trade_pnl(trade: dict) -> dict:
    ...
    if current_price is None:
        # sem preço disponível, mantém como está
        return trade
    ...
    if side == "BUY":
        # comprámos a `entry`, vale agora `current_price`
        # tokens comprados = size / entry
        tokens = size / entry
        current_value = tokens * current_price
        trade["pnl"] = round(current_value - size, 4)
    else:
        # vendemos a `entry`, se preço caiu lucramos
